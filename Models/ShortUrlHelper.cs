...
public static class ShortUrlHelper
{
    ...
    public async static Task<bool> ShortUrlAlreadyExists(CloudStorageAccount storageAccount, string shortUrl)
    {
        var tableClient = storageAccount.CreateCloudTableClient();
        var urlTable = tableClient.GetTableReference("url");
        await urlTable.CreateIfNotExistsAsync();
        var query = new TableQuery<ShortUrlEntity>().Where(TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, shortUrl));
        var shortUrls = urlTable.ExecuteQuery(query);
        return shortUrls.Any();
    }

    public static void SaveShortUrl(CloudStorageAccount storageAccount, ShortUrlEntity url)
    {
        var tableClient = storageAccount.CreateCloudTableClient();
        var urlTable = tableClient.GetTableReference("url");
        bool created = urlTable.CreateIfNotExists();
        var insertOperation = TableOperation.Insert(url);
        urlTable.Execute(insertOperation);
    }

    public async static Task<string> GetRealUrl(CloudStorageAccount storageAccount, string shortUrl)
    {
        var tableClient = storageAccount.CreateCloudTableClient();
        var urlTable = tableClient.GetTableReference("url");
        await urlTable.CreateIfNotExistsAsync();
        var query = new TableQuery<ShortUrlEntity>().Where(TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, shortUrl));
        var result = urlTable.ExecuteQuery(query).FirstOrDefault();
        return result != null ? result.RealUrl : string.Empty;
    }
    ...
    public static string RandomCharacters()
    {
        ...
        var shortUrl = new char[random.Next(5, 5)];
